...
Deutschland <- read_html(url) %>%
  html_nodes('.textBox > div:nth-child(1) > table:nth-child(4)') %>%
  html_table(fill = TRUE) %>%
  as.data.frame()
...
date <- read_html(url) %>%
  html_nodes(".textBox > div:nth-child(1) > p:nth-child(3)") %>%
  html_text() %>%
...
pays <- read.csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv") %>%
  select(-1)
